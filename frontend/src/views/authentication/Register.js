...
const Register = () => {
  ...
  const handleSubmit = async ({ name, email, password, confirm_password, role }) => {
    if (!profileImage) {
      toast.error('Please capture your face for identity verification');
      return;
    }
    ...
    if (password !== confirm_password) {
      ...
      try {
        const res = await register({ name, email, password, role, profileImage }).unwrap();
