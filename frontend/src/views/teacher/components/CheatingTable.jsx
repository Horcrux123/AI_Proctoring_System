...
import { useGetCheatingLogsQuery, useDeleteCheatingLogsMutation } from 'src/slices/cheatingLogApiSlice';
...
export default function CheatingTable() {
  ...
  const [deleteCheatingLogs, { isLoading: isDeleting }] = useDeleteCheatingLogsMutation();

  const handleClearLogs = async () => {
    if (window.confirm("Are you sure you want to clear all logs for this exam?")) {
      try {
        await deleteCheatingLogs(selectedExamId).unwrap();
        alert("Logs cleared successfully.");
        refetchLogs();
      } catch (err) {
        alert("Failed to clear logs: " + (err?.data?.message || err.error || 'Unknown error'));
      }
    }
  };
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={2}>
            <Button
              ...
              color="error"
              fullWidth
              disabled={!selectedExamId || isDeleting || cheatingLogs.length === 0}
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Prohibited Object</TableCell>
                <TableCell>Identity Mismatch</TableCell>
                <TableCell>Tab Switched</TableCell>
                <TableCell>Exited Fullscrn</TableCell>
                <TableCell>Voice Detected</TableCell>
                <TableCell>Eye Gaze</TableCell>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((log, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.identityMismatchCount)}
                        label={log.identityMismatchCount || 0}
                        color={getViolationColor(log.identityMismatchCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.tabSwitchedCount)}
                        label={log.tabSwitchedCount || 0}
                        color={getViolationColor(log.tabSwitchedCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.exitedFullscreenCount)}
                        label={log.exitedFullscreenCount || 0}
                        color={getViolationColor(log.exitedFullscreenCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.voiceDetectedCount)}
                        label={log.voiceDetectedCount || 0}
                        color={getViolationColor(log.voiceDetectedCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.eyeGazeCount)}
                        label={log.eyeGazeCount || 0}
                        color={getViolationColor(log.eyeGazeCount || 0)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
